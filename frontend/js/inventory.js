...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function openAddModal() {
        ...
        // applies to CHANGING an existing product's batch_number.
        const batchEl = document.getElementById('field-batch_number');
        if (batchEl) {
    ...
    async function openEditModal(id) {
        ...
        // Only super_admin ("the owner") may change a batch_number once it's
        // been set — admins can view it here but the field is locked.
        const batchEl = document.getElementById('field-batch_number');
        if (batchEl) {
            const isOwner = Auth.getUser()?.role === 'super_admin';
            batchEl.disabled = !isOwner;
            batchEl.title    = isOwner ? '' : 'Only the owner (super admin) can change the batch number.';
        }
    ...
    async function handleFormSubmit() {
        ...
        // Standard confirmation step before any inventory write — shows exactly
        // what's about to be saved so a mis-typed price/quantity gets caught
        // before it hits the database.
        const summary =
            `Batch: ${body.batch_number}\n` +
            `Name: ${body.name}${body.generic_name ? ' (' + body.generic_name + ')' : ''}\n` +
